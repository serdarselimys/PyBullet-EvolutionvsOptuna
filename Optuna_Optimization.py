...
from live_queue_dashboard import QueueDashboardLogger, DashboardQueueListener
...
# 1. SETTINGS & GLOBALS
# ============================================================
NUM_ROBOTS = 10		# = Number of robots (also = Optuna trials asked per generation)
DURATION = 10		# = Lenght of each trial in seconds
SPACING = 1.5		#Distance between each robot
GAIT_TRIALS = 30		# = number of Optuna generations (each generation asks NUM_ROBOTS trials)
NUM_EXPERIMENTS = 10
...
# --- Optuna sampler: "tpe" is the default Tree-structured Parzen Estimator sampler,
#     "cmaes" uses CMA-ES, generally stronger for small continuous search spaces like this one ---
SAMPLER_MODE = "tpe"
_SAMPLER_CHOICES = ("tpe", "cmaes")

# --- Live TensorBoard-style dashboard: logs score/DNA/metric curves per generation
#     to DASHBOARD_LOGDIR, one subfolder per experiment. Most useful in "headless"
#     mode where there's no PyBullet GUI to watch. View with `python launch_dashboard.py`.
ENABLE_DASHBOARD = True
DASHBOARD_LOGDIR = "runs"
...
# --- Gene / Optuna search space: (low, high) ---
GENE_BOUNDS = {
...
# Spin-mode leg roots
LEG_ROOTS = {
    "FL": [0.12, 0.10], "ML": [0.0, 0.10], "RL": [-0.12, 0.10],
    "FR": [0.12, -0.10], "MR": [0.0, -0.10], "RR": [-0.12, -0.10],
}
...
assert SAMPLER_MODE in _SAMPLER_CHOICES, f"SAMPLER_MODE must be one of {_SAMPLER_CHOICES}"

def make_optuna_sampler(mode):
    import optuna
    if mode == "cmaes":
...
def hold_stance(robots, joint_map, leg_prefixes, body_height, seconds, client_id):
    h_set, th_set, kn_set = solve_leg_ik_3dof(0, 0, body_height)
    if h_set is None:
        return
...
# ============================================================
# 6. WORKER (OPTUNA INTEGRATED, MULTI-MODE)
# ============================================================

def run_experiment_worker(args):
    import optuna

    mode, gait_name, gait_offsets, target_vel, body_height, step_height, direction, exp_idx, dashboard_queue = args
    np.random.seed()

    study = optuna.create_study(direction="maximize", sampler=make_optuna_sampler(SAMPLER_MODE))

    group_key = f"Optuna_{SAMPLER_MODE.upper()}_{mode}_dir{direction}_h{body_height}"
    dashboard = QueueDashboardLogger(dashboard_queue, group_key, exp_idx, enabled=ENABLE_DASHBOARD)
    ...
    for gen in range(GAIT_TRIALS):
        ...
        # --- Ask Optuna for NUM_ROBOTS trials, one per robot, for this generation ---
        trials = [study.ask() for _ in range(NUM_ROBOTS)]
        population = [
            {
                "step_amplitude": trial.suggest_float("step_amplitude", *GENE_BOUNDS["step_amplitude"]),
                "frequency":      trial.suggest_float("frequency", *GENE_BOUNDS["frequency"]),
            }
            for trial in trials
        ]
            ...
            if s % servo_interval == 0:
                for i, r_id in enumerate(robots):
                    ...
                    for l_idx, prefix in enumerate(LEG_PREFIXES):
                        ...
                        h, th, kn = solve_leg_ik_3dof(tx, ty, tz)
...
if __name__ == "__main__":
    ...
    print(f"\n=== Running Optuna optimization for modes: {MODES}  "
          f"(render: {RENDER_MODE}, sampler: {SAMPLER_MODE}) ===")

    # A single shared queue lets all worker processes report their per-generation
    # results to one listener in this main process, which combines repeats of the
    # same config into ONE TensorBoard run — no individual per-experiment run
    # directories are ever written to disk.
    dashboard_manager = None
    dashboard_queue = None
    listener = None
    if ENABLE_DASHBOARD:
        dashboard_manager = multiprocessing.Manager()
        dashboard_queue = dashboard_manager.Queue()
        listener = DashboardQueueListener(dashboard_queue, log_root=DASHBOARD_LOGDIR)
        listener.start()
                        ...
                        for direction in dirs_for_mode:
                            for i in range(NUM_EXPERIMENTS):
                                tasks.append((
                                    ...
                                    b_height, step_height, direction, i, dashboard_queue
                                ))
    ...
    if listener is not None:
        listener.stop()  # drains any remaining messages, then closes cleanly
